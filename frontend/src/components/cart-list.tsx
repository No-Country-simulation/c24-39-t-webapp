...
export default function CartList({ session }: CartListProps) {
    const { globalCart, carts, removeFromCart, addToCart, clearCart } = useCart();
    ...
    const submitOrder = async () => {
        ...
        try {
            ...
            const simulatedResponse: OrderResponse = {
                ord_id: Math.floor(Math.random() * 1000), // ID aleatorio para simulación
                clientId: session!.user.id,
                restaurantId: parseInt(restaurantId),
                state: "pendiente",
                total,
                comments,
                details: orderRequest.details.map((detail, index) => ({
                    odt_id: index + 1,
    ...
    return (
        <div className="w-full max-w-4xl mx-auto mb-12 p-6 bg-cream rounded-lg shadow-lg min-h-screen">
            ...
            {globalCart.length > 0 ? (
                <div className="space-y-4">
                    ...
                    <button
                        ...
                        className={`mt-6 w-full py-3 text-white font-semibold rounded-md ${isSubmitting || globalCart.length === 0
